...
! Parameters:  
 ...
 double precision  :: x(4,3),tsing(3),bmat(3),f_t,amat(3,3),mass(4),total_mass,n3(1:3),v12(3),v23(3),v31(3)
 !
 ...
 do while( yes )
    ...
    if ( trim(longlabel)/='end'.and.trim(longlabel)/='END') then
       !
       !
       read (longlabel,*) x(4,1:3),x(1,1:3),x(2,1:3),x(3,1:3),mu(1),mu(2),mu(3)
       !
       x(1,:) = x(1,:) - x(4,:)
	   x(2,:) = x(2,:) - x(4,:)
	   x(3,:) = x(3,:) - x(4,:)
       ...
       write (f_out,"(3f10.6,3f10.4,3(3f14.8,5x,3f14.8))") & 
            local(1:3),local(4:6)*180.d0/pi,&
            edip(1:3,:),mu(1:3),mu(1:3)-edip(1:3,1)
       !
    else
       !
       yes = .false.
